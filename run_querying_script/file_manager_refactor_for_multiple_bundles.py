...
# helper: takes in PDF of bundled articles and dict; adds articles into dict with title as key and raw text as value
def articles_to_dict(bundle_name, dict):
    # temp accumulator stores text so we can enter it into dict after article ends:
    acc = ""

    # Open up our pdf & begin iterating!
    doc = pymupdf.open(f"{full_path}/{bundle_name}") 
    for page in doc:
        text = page.get_text()
        acc += text
        ...
        if "Citation (aglc Style)\n" in text:
            ...
            dict[article_title] = article_data(project_name, acc)  # Store accumulated text
...
for plant_folder in folders_list:
    ...
    if len(pdfs_list) > 1:
        ...
        for pdf in pdfs_list:
            title_text_dict = {}
            articles_to_dict(pdf, title_text_dict)
   
    if len(pdfs_list) == 1 and "NewsBank" in pdfs_list[0]:
        # Unzips current pdf bundle & turns it into dict where key = article name and value = raw article text
        article_bundle_pdf = pdfs_list[0]
        title_text_dict = {}
        articles_to_dict(article_bundle_pdf, title_text_dict)
