...
# DEFINING OUTPUT (FLATMAP) DF:
out_df = pd.DataFrame(columns=['Document_ID', 'File_stored', 'link', 'unique_id', 'pdf_text'])
        ...
        if pdf_paths is not None:
            ...
            for idx, pdf_path in enumerate(pdf_paths):
                try:
                    ...
                    pdf_text = extract(local_pdf)

                except Exception as e:
                    # print(f"Error processing {pdf_path}: {e}")
                    pdf_text = "No text found."

                # insert PDF text as new row in out_df:
                out_df.loc[len(out_df)] = [doc_id, 
                                           col_name, 
                                           pdf_path, 
                                           f"{doc_id}_{col_name}_{idx}", 
                                           pdf_text]
...
out_df['metadata_input_path'] = in_csv_path
# (3) output as csv!
out_df.to_csv(f'out_{timestamp}.csv', index=False)