# Legal case classification prompts for LangGraph workflow
# These prompts match the tasks from summary_chunked_instructor.py

# System prompt template
system_prompt = """We are social scientists working at the intersection of sociology (our home discipline) and political science (a secondary focus) trying to understand how federal judges rule on civil cases related to environmental lawsuits. Here is the text of a judicial opinion issued by a federal district court judge on an environmentally-related dispute:

{decision_text}

This judicial opinion is associated with the following unique ID:

{decision_id}"""

# Task 1: Legal conflict identification
legal_conflict_prompt = """{system_prompt}
...
# Task 2: Plaintiff arguments summary
plaintiff_arguments_prompt = """{system_prompt}

Task 2. What are the main arguments made by the plaintiff(s) in this case? While there is no formal limit to the length of each summary, try to keep each one to less than 250 words, although also keep in mind that summaries with fewer than 50 words may start not to capture the essence of the argument with enough nuance and depth.

Please provide only the summary of the plaintiff arguments in a clear, concise format."""

# Task 3: Plaintiff argument classification
plaintiff_classification_prompt = """{system_prompt}

Plaintiff arguments summary: {plaintiff_arguments}

IMPORTANT: You must respond with ONLY valid JSON. Do not include any text before or after the JSON object.

Task 3. Parse the plaintiff arguments above and classify each individual argument separately. The principle themes or guiding logic of the arguments that we expect to encounter are listed below. For each individual plaintiff argument (numbered 1, 2, 3, etc.) and each possible classification found below, please calculate a relevance score according to the scoring methodology provided.
...
{{
  "argument_1": {{
    "cost reasoning": [true, 0.9, "The argument frequently discusses 'county revenue' and 'jobs creation' related to the project."],
    "ecological reasoning": [false, 0.1, ""],
    "health reasoning": [false, 0.0, ""],
    "Major Federal Action Reasoning": [false, 0.0, ""],
    "Missing Congressional Approval reasoning": [false, 0.0, ""],
    "Omission Reasoning": [false, 0.0, ""],
    "Valid Environmental Analysis Reasoning": [false, 0.0, ""],
    "Feasible Alternative Reasoning": [false, 0.0, ""],
    "Good Faith Analysis Reasoning": [false, 0.0, ""],
    "Consultation Reasoning": [false, 0.0, ""],
    "Undue burden reasoning": [false, 0.0, ""],
    "Equality reasoning": [false, 0.0, ""],
    "federalism reasoning": [false, 0.0, ""],
    "deferential reasoning": [false, 0.0, ""],
    "conservative reasoning": [false, 0.0, ""],
    "Military reasoning": [false, 0.0, ""],
    "standing reasoning": [false, 0.0, ""],
    "Recreation reasoning": [false, 0.0, ""],
    "Retroactive reasoning": [false, 0.0, ""],
    "Legal Technicality Reasoning": [false, 0.1, ""]
  }},
  "argument_2": {{
    "cost reasoning": [false, 0.0, ""],
    ...
    "Recreation reasoning": [false, 0.0, ""],
    "Retroactive reasoning": [false, 0.0, ""],
    "Legal Technicality Reasoning": [false, 0.2, ""]
  }}
}}
...
Please provide the classification for each individual plaintiff argument as a JSON object. """



# Task 4: Defendant arguments summary
defendant_arguments_prompt = """{system_prompt}

Task 4. What are the main arguments made by the defendant(s) in this case? While there is no formal limit to the length of each summary, try to keep each one to less than 250 words, although also keep in mind that summaries with fewer than 50 words may start not to capture the essence of the argument with enough nuance and depth.

Please provide only the summary of the defendant arguments in a clear, concise format."""

# Task 5: Defendant argument classification
defendant_classification_prompt = """{system_prompt}

IMPORTANT: You must respond with ONLY valid JSON. Do not include any text before or after the JSON object.

Defendant arguments summary: {defendant_arguments}
...
Please provide the classification for each individual defendant argument as a JSON object. 

"""

# Task 6: Judge arguments summary
judge_arguments_prompt = """{system_prompt}

Task 6. What are the main arguments made by the judge(s) in this case? While there is no formal limit to the length of each summary, try to keep each one to less than 250 words, although also keep in mind that summaries with fewer than 50 words may start not to capture the essence of the argument with enough nuance and depth.

Please provide only the summary of the judge arguments in a clear, concise format."""

# Task 7: Judge argument classification
judge_classification_prompt = """{system_prompt}

IMPORTANT: You must respond with ONLY valid JSON. Do not include any text before or after the JSON object.

Judge arguments summary: {judge_arguments}

Task 7. Parse the judge arguments above and classify each individual argument separately. The principle themes or guiding logic of the arguments that we expect to encounter are listed below. For each individual judge argument (numbered 1, 2, 3, etc.) and each possible classification found below, please calculate a relevance score according to the scoring methodology provided. 
